...
ADD COLUMN discription VARCHAR(100);
...
CREATE OR REPLACE FUNCTION UDF_asses(IN saleid INT)
RETURNS REFCURSOR
AS
$$
	...
	BEGIN
		OPEN ref1 FOR SELECT product_name,category_name,user_name,product_price,(
		CASE
...
CREATE OR REPLACE PROCEDURE USP_upFETCH(IN usr_id INT,OUT Ret REFCURSOR)
LANGUAGE PLPGSQL
AS
$$
	BEGIN
		...
		IF((usr_id IS NULL) OR (EXISTS (SELECT user_id FROM tblusers WHERE user_id = usr_id))) THEN

			OPEN Ret FOR SELECT category_name,product_name
			FROM tblusers tu
			JOIN tblsales ts
			ON ts.sales_user_id = tu.user_id
			JOIN tblProducts tp
